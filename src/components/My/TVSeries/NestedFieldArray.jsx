...
export const NestedFieldArray = ({control, register}) => {
    ...
    const addSeason = () => {
        append({description: '', numberOfEpisodes: 10, opinion: '', status: 'toWatch'})
    }
            ...
            {fields.map((item, index) => {
                return (
                    <div key={item.id} >
                        ...
                        <input ref={register({required: false})} name={`seasons[${index}].opinion`} 
                        control={control} defaultValue={item.opinion} type="text" />
                        <input control={control} defaultValue={item.description} name={`seasons.${index}.description`} ref={register({required: false})} type="text" />
                        <button onClick={removeItem(index)} type="button">Delete</button>
                        </div>

...
const MovieForm = (props) => {
    ...
    return (
        ...
        <AddMovieForm 
            initialValues={{
                ...movie, 
                cathegory: movie.cathegory.map((cathegory) => GENRE_OPTIONS.filter((option) => option.value === cathegory)[0]),
                status: MOVIE_STATUS.filter((status => status.value === movie.status))[0]
            }} 
        />
        <div>
          <button className="movie-form__menu" onClick={goToPrevMovie} type="button">Go To Prev Movie</button>
          <button className="movie-form__menu" onClick={goToNextMovie} type="button">Go To Next Movie</button>
          <button className="movie-form__menu" onClick={saveMovie} type="button">Save</button>
        </div>
        </>
    )
}
...
const mapStateToProps = (state) => ({
  myValues: formValues(state, 'title', 'titlePl', 'year', 'cathegory', 'isBook', 'isSaga', 'status', 'date', 'ranking', 'isWorthWatchingAgain', 'nrOfTimesWatched', "directors", "remarks")
})
